...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % Computes softmax derivative
  % Requires a for loop and is non vectorizable if performed in 2 steps
  % J = diag(current_prob) - current_prob * current_prob';
  % dE_dp = current_target ./ current_prob; % 1x10
  % dE_dc = J * dE_dp; % 1x10

  % The derrivative to the error with respect to the class input simplifies to this
  % Thanks to the appropriate choice of error function
  dE_dc = data.targets - class_prob;
  dE_dW2 = dE_dc * hid_output'; % 10xH

  dE_dh = dE_dc' * model.hid_to_class; % NxH
  dE_da = dE_dh' .* hid_output .* (1 - hid_output); % NxH
  dE_dW1 = dE_da * data.inputs'; % HxD
  
  ret.input_to_hid = - 1 / N * dE_dW1 + wd_coefficient * model.input_to_hid;
  ret.hid_to_class = - 1 / N * dE_dW2 + wd_coefficient * model.hid_to_class;
end
